...
import type { Candidate, PersonPayload, SearchEvent } from "./types";
...
export async function runSearch(
  ...
  await run("INSERT INTO searches (id, query) VALUES (?, ?)", [
    searchId,
    query,
  ]);
  ...
  try {
    ...
    if (intent.route !== "academic") {
      throw new Error(
        `This build only handles academic searches. That query looks like a "${intent.route}" search — try naming a university and a research area.`,
      );
    }
    ...
    candidates = await rerank(intent, candidates);
      ...
      candidates.map((c, i) =>
        limit(async () => {
          ...
          try {
            ...
            emit({ type: "person", person: payload });
          } catch (err) {
